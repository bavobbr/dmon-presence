...
<div class="table-wrap"><table><thead><tr><th>Player</th><th onclick={()=>setSort('score')} title={matchSource==='official'?'Uses official Sportlink appearances':'Uses declared Twizzit availability'}>{scoreHeading}</th><th onclick={()=>setSort('training')}>Training presence</th><th onclick={()=>setSort('appearances')}>Official appearances</th><th onclick={()=>setSort('availability')}>Match availability</th><th onclick={()=>setSort('homeAppearances')}>Home appearances</th><th onclick={()=>setSort('awayAppearances')}>Away appearances</th><th onclick={()=>setSort('overallAdmin')}>Admin</th><th>Objective insights</th></tr></thead><tbody>
{#each rows as r}<tr><td><a href="/players/{r.id}?dataset={data.datasetId}"><strong>{r.firstName} {r.lastName}</strong></a>{#if r.kind==='substitute'} <span class="pill warn">Substitute</span>{/if}</td><td class="score">{score(r)==null?'N/A':`${score(r).toFixed(1)}%`}</td><td>{formatMetric(r.metrics.training)}</td><td>{formatMetric(r.metrics.appearances)}</td><td>{formatMetric(r.metrics.availability)}</td><td>{formatMetric(r.metrics.homeAppearances)}</td><td>{formatMetric(r.metrics.awayAppearances)}</td><td title={`Training: ${formatMetric(r.metrics.trainingAdmin)} · Match: ${formatMetric(r.metrics.matchAdmin)} · Overall: ${formatMetric(r.metrics.overallAdmin)}`}>{formatMetric(r.metrics.overallAdmin)}</td><td>
